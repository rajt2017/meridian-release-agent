# Meridian Data Platform API — Reference (v2)

**Release:** Current codebase (post Q3 2025)  
**Base URL:** `https://api.meridian.example/v2`  
**Auth:** Pass your API key in the `X-API-Key` header on every request.

```bash
curl -H "X-API-Key: $MERIDIAN_API_KEY" \
  "https://api.meridian.example/v2/list-records"
```
...
const client = new MeridianClient({
  baseUrl: "https://api.meridian.example/v2",
...
| `sort_order` | query | no       | Sort by `created_at`: `asc` or `desc`            |

**Example**

```bash
curl -H "X-API-Key: $MERIDIAN_API_KEY" \
  "https://api.meridian.example/v2/list-records?filter=active&sort_order=desc"
```
...
```bash
curl -H "X-API-Key: $MERIDIAN_API_KEY" \
  "https://api.meridian.example/v2/get-profile?user_id=usr_1001"
```
...
curl -X POST \
  ...
  "https://api.meridian.example/v2/create-record"
```
...
Searches records by name. **Returns exact matches only** — partial and fuzzy matching are not supported.
...
```bash
curl -H "X-API-Key: $MERIDIAN_API_KEY" \
  "https://api.meridian.example/v2/search?q=customer-events"
```
